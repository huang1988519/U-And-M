...
-(void)configVoiceSwitchBar
{
 ...
 /*
    ...
    [voiceSwitch setOnImage:[UIImage imageNamed:@"voice_on.png"]];
    [voiceSwitch setOffImage:[UIImage imageNamed:@"voice_off.png"]];

    [voiceSwitch addTarget:self action:@selector(voiceStateChange:) forControlEvents:UIControlEventValueChanged];
    [voiceSwitch setOn:NO];
    _switch = voiceSwitch;
    
    UIBarButtonItem * item = [[UIBarButtonItem alloc] initWithCustomView:voiceSwitch];
    UINavigationItem * naviItem = self.navigationItem;
    [naviItem setRightBarButtonItem:item];
